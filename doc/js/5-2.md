...
![image](http://zff-img.lshwn.cn/blog/js/5-2/1.jpg)
...
function cloneDeep4(source) {
    const hasMap = getWeakMap();// 使用weakMap用来做数据缓存
    // root 即为最终复制产出的target值
    const root = {};

    // 定义初始状态的栈，父指针指向root对象
    const loopList = [
        {
            parent: root,
            key: undefined,
